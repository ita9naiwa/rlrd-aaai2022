...
for num_tasks in [args.num_task]:
    ...
    for util in util_range:
        ...
        for x, y in train_dataset:
            ...
            res['DkC_RTA_LC'] += (heu.test_RTA_LC(y, num_proc, p, use_deadline))
            ...
            res['DM_DS_RTA_LC'] += (heu.test_RTA_LC(y, num_proc, p, use_deadline))
            res['DM_DS_DA_LC'] += (heu.test_DA_LC(y, num_proc, p, use_deadline))

            p = scores_to_priority(heu.get_SM_DS_scores(y, num_proc, use_deadline=use_deadline))
            res['SM_DS_RTA_LC'] += (heu.test_RTA_LC(y, num_proc, p, use_deadline))
            res['SM_DS_DA_LC'] += (heu.test_DA_LC(y, num_proc, p, use_deadline))
        ...
        if use_deadline:
            with open("repo/heuristics_result_%d_dl.pkl" % (num_proc), 'wb') as ff:
                pickle.dump(dict(res_map), ff)
        else:
            with open("repo/heuristics_result_%d.pkl" % (num_proc), 'wb') as ff:
                pickle.dump(dict(res_map), ff)
